...
```lua
function debug.getconstant(func: function, index: number): any
```

Returns the constant at `index` in the constant table of the function `func`. Throws an error if the constant does not exist.

#### Parameters
* `func`: A lua function of which to get constants from.
...
```lua
function debug.getconstants(func: function | number): array<any>
```
...
Refer to the [DebugInfo](#debuginfo) data type to see what it contains.
...
```lua
function debug.getproto(func: function, index: number, active: boolean?): function | array<function> | nil
```

Returns the proto at `index` in the function `func` if `active` is false.

If `active` is `true`, then every active function of the proto is returned.

#### Parameters
* `func`: The lua function to target.
...
```lua
function debug.getprotos(func: function | number): array<function>
```
...
```lua
function debug.getstack(level: number, index: number?): any | array<any>
```
